...
public class StreamingAI extends JFrame {
	...
	public boolean oneMeasureFinish = true;
	...
	class DataReadyEventListener implements BfdAiEventListener{
		public void BfdAiEvent(Object sender, BfdAiEventArgs args){
			...
			// write to file
			printWriter.println(Integer.toString(serialNum++)+"   "+Double.toString(dataScaled[0])+"   "+Double.toString(dataScaled[1]));
			...
			if (oneMeasureFinish ) {
		        ...
		        if (!file.exists()) {
		        	 try {
							...
							printWriter.println("序号"+"           "+"REF1"+"           "+"REF2");	
